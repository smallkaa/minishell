...
static void	handle_out_redirection(t_cmd *cmd)
{
	...
	if (cmd->out_redir->type == R_OUTPUT)
		...
		fatal_error_child("redirection", EXIT_FAILURE);
	out = open(cmd->out_redir->filename, mode, 0644);
	if (out < 0)
		fatal_error_child("write", EXIT_FAILURE);
	if (dup2(out, STDOUT_FILENO) == -1)
	{
		if (close(out) == -1)
			fatal_error_child("close", EXIT_FAILURE);
		fatal_error_child("dup2", EXIT_FAILURE);
	}
	if (close(out) == -1)
		fatal_error_child("close", EXIT_FAILURE);
}
...
static void	handle_in_redirection(t_cmd *cmd)
{
	...
	if (!cmd->in_redir)
		fatal_error_child("cmd->in_redir", EXIT_FAILURE);
	in = open(cmd->in_redir->filename, O_RDONLY);
	if (in < 0)
		fatal_error_child("read", EXIT_FAILURE);
